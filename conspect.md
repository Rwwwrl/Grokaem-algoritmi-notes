...
1. **Использовать список**, а именно методы _append_ и _pop_, **но!**
   Список изначально был сделан для произовольного доступа к элементам, это значит что весь список это единный объем памяти, если у нас огромный список, то добавление в конец может происходить дольше чем обычно. В этом случае **мы используем то, что нам не нужно!**
2. **deque** - очередь, основанная на двухсвязнном списке.
...
скорость работы - **O(n log(n)) < work < O(n^2)**, она зависит от выбора опорного элемента.
...
Есть пустой массив [], есть функция конвертирующая _объект_ (строку, число, экземпляр - неважно) в число. Это число определяет индекс в массиве куда и заносится наш _объект_. Именно из-за того что мы используем массив, скорость чтения в словаре - **_O(1)_** .  
...
# когда продавец находится на втором уровне
...
В предыдущей главе мы рассматривали _поиск в ширину_. Этот алгортим находит путь с _минимальным количество сегментов_, но что если к каждому сегменту у нас еще будет привязано какое то число, допустим время (вес этого сегмента). В таком случае кратчайшим (наибыстрейшим) расстоянием будет не минимальное количество ребер, а минимальное время затраченное на путь. Быстрее всего поиск такого пути делается при помощи **_алгоритма Дейкстры_**.
...
class Graph:
    ...
    def _get_actualized_graph(self, start_position: int):
        '''
        тут мы убираем те точки графа, до которыx не сможем дотянуться от стартовой позиции
...
| -------- | --------- | ---------------------- |
| Поиск    | O(log(n)) | O(log(h))              |
| Вставка  | O(n)      | O(log(h))              |
| Удаление | O(n)      | O(log(h))              |
